// ABOUTME: Configuration loading system for holodeck-coordinator service
...
pub struct ServiceConfig {
    ...
    pub orchestration_ai: OrchestrationAiConfig,
}
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct OrchestrationAiConfig {
    pub response_time_target_ms: u64,
    pub service_coordination_timeout_ms: u64,
    pub workflow_execution_timeout_ms: u64,
    pub performance_optimization_enabled: bool,
    pub health_monitoring_interval_ms: u64,
    pub max_concurrent_orchestrations: u32,
    pub enable_advanced_workflow_management: bool,
    pub enable_distributed_validation: bool,
}
...
enable_distributed_validation = true
        ...
        assert_eq!(config.service.name, "test-coordinator");
        assert_eq!(config.llm.provider, "ollama");
        assert_eq!(config.llm.model, "gemma2:2b");
        assert_eq!(config.orchestration_ai.response_time_target_ms, 100);
        assert_eq!(config.orchestration_ai.max_concurrent_orchestrations, 100);
        assert!(config.orchestration_ai.performance_optimization_enabled);
    }
    ...
    fn test_llm_config_conversion() {
        let service_config = ServiceConfig {
            ...
            orchestration_ai: OrchestrationAiConfig {
                response_time_target_ms: 100,
                service_coordination_timeout_ms: 5000,
                workflow_execution_timeout_ms: 30000,
                performance_optimization_enabled: true,
