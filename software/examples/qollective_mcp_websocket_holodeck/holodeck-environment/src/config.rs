// ABOUTME: Configuration loading system for holodeck-environment service
...
pub struct ServiceConfig {
    ...
    pub environment_ai: EnvironmentAiConfig,
}
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct EnvironmentAiConfig {
    pub response_time_target_ms: u64,
    pub environment_cache_hours: u64,
    pub physics_simulation_quality: String,
    pub safety_validation_level: String,
}
...
physics_simulation_quality = "high"
safety_validation_level = "strict"
        "#;

        let config: ServiceConfig = toml::from_str(config_toml).unwrap();
        assert_eq!(config.service.name, "test-environment-service");
        assert_eq!(config.llm.provider, "ollama");
        assert_eq!(config.llm.model, "gemma2:2b");
        assert_eq!(config.llm.max_tokens, 3500);
        assert_eq!(config.environment_ai.response_time_target_ms, 500);
        assert_eq!(config.environment_ai.safety_validation_level, "strict");
    }
    ...
    fn test_llm_config_conversion() {
        let service_config = ServiceConfig {
            ...
            environment_ai: EnvironmentAiConfig {
                response_time_target_ms: 500,
                environment_cache_hours: 1,
        ...
        assert_eq!(llm_config.max_tokens, Some(3500));
    }
}