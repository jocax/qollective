 ...
 * Matches backend GenerationEvent structure with camelCase serialization
 */
export interface GenerationEvent {
	eventType: string // "Started", "Progress", "Completed", "Failed", "ToolExecution"
	tenantId: string
	requestId: string
	timestamp: string // ISO 8601 format
	servicePhase: string // "prompt-helper", "story-generator", "quality-control", "constraint-enforcer"
...
/**
 * Tracked request from backend monitoring system
 * Returned by get_active_requests command
 * Matches backend TrackedRequest with camelCase serialization
 */
export interface TrackedRequest {
	requestId: string
	tenantId: string
	startTime: string // ISO 8601 format from DateTime<Utc>
	currentPhase: string
	progress: number // 0.0 to 1.0
	lastUpdate: string // ISO 8601 format from DateTime<Utc>
	component: string
	status: string // "in_progress", "completed", "failed"
	errorMessage?: string
	filePath?: string
