...
pub mod test_utils {
    ...
    pub fn validate_with_details(schema: &Validator, data: &Value) -> Result<(), Vec<String>> {
    ...
    pub fn run_validation_test(
        ...
        let passed = if should_pass { is_valid } else { !is_valid };

        let error_message = if !passed {
            if should_pass {
                let errors: Vec<String> = schema.iter_errors(data).map(|e| e.to_string()).collect();
                Some(format!(
...
mod integration_tests {
    ...
    fn test_envelope_integrates_with_all_meta_schemas() {
        let schema_dir = Path::new("../schemas/core");
        let envelope_schema =
            load_schema(&schema_dir.join("envelope.json")).expect("Failed to load envelope schema");
        ...
        assert!(
            ...
            envelope_schema
                .iter_errors(&comprehensive_envelope)
                .collect::<Vec<_>>()
        );
    }
    ...
    fn test_cross_schema_consistency() {
        ...
        let security_schema = load_schema(&schema_dir.join("metadata/security.json")).unwrap();
        ...
        let envelope_with_security = json!({
            ...
            "payload": {"test": "security integration"}
        });
        ...
        assert!(
            ...
            envelope_schema
                .iter_errors(&envelope_with_security)
                .collect::<Vec<_>>()
        );
    }
}
