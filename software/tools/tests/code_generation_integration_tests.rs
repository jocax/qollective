...
use qollective_tools_lib::SchemaParser;
use serde_json::json;
use std::fs;
...
mod code_generation_integration_tests {
    ...
    fn test_envelope_schema_generates_complete_rust_code() {
        ...
        fs::write(
            &schema_file,
            serde_json::to_string_pretty(&envelope_schema).unwrap(),
        )
        .unwrap();
        ...
        assert!(
            generated_code.contains("pub struct QollectiveEnvelope"),
            "Should generate main envelope struct"
        );

        // Should contain meta struct
        assert!(
            generated_code.contains("pub struct Meta") || generated_code.contains("meta:"),
            "Should include meta field or struct"
        );

        // Should contain error struct
        assert!(
            generated_code.contains("pub struct Error") || generated_code.contains("error:"),
            "Should include error field or struct"
        );

        // Should have proper serde imports and derives (typify uses full paths)
        assert!(
            generated_code.contains(":: serde ::") || generated_code.contains("use serde"),
            "Should import serde"
        );
        assert!(
            generated_code.contains("Serialize"),
            "Should derive Serialize"
        );
        assert!(
            generated_code.contains("Deserialize"),
        ...
        assert!(
            ...
            "Should use Option for optional fields"
        );

        // Should compile without errors
        assert!(
            validate_rust_syntax(&generated_code),
            "Generated code should have valid Rust syntax"
        );
    }
    ...
    fn test_metadata_schema_generates_specialized_types() {
        ...
        fs::write(
            &schema_file,
            serde_json::to_string_pretty(&security_meta_schema).unwrap(),
        )
        .unwrap();
        ...
        assert!(
            result.is_ok(),
            "Security meta schema generation should succeed"
        );
        ...
        assert!(
            generated_code.contains("SecurityMeta"),
            "Should generate SecurityMeta struct"
        );

        // Should contain auth method enum (typify generates SecurityMetaAuthMethod)
        assert!(
            generated_code.contains("SecurityMetaAuthMethod"),
            "Should generate SecurityMetaAuthMethod enum"
        );

        // Enum should have proper variants (typify converts to PascalCase)
        assert!(
            generated_code.contains("Jwt") || generated_code.contains("JWT"),
            "Should have JWT variant"
        );
        assert!(
            generated_code.contains("OAuth") || generated_code.contains("Oauth"),
            "Should have OAuth variant"
        );
        assert!(
            generated_code.contains("ApiKey") || generated_code.contains("api_key"),
            "Should have API key variant"
        );

        // Should handle arrays properly (typify generates Vec<SecurityMetaPermissionsItem>)
        assert!(
            generated_code.contains("SecurityMetaPermissionsItem")
                || (generated_code.contains("Vec<") && generated_code.contains("permissions")),
            "Should handle permissions array"
        );

        // Should handle required vs optional fields properly (typify generates proper types)
        assert!(
            generated_code.contains("user_id"),
            "Should have user_id field"
        );
        assert!(
            generated_code.contains("auth_method"),
            "Should have auth_method field"
        );

        // Optional fields should be wrapped in Option (typify uses full paths)
        assert!(
            generated_code.contains("Option <") || generated_code.contains("Option<"),
            "Optional fields should be Option types"
        );
    }
    ...
    fn test_complex_nested_schema_generation() {
        ...
        fs::write(
            &schema_file,
            serde_json::to_string_pretty(&complex_schema).unwrap(),
        )
        .unwrap();
        ...
        assert!(
            generated_code.contains("pub struct ServiceConfig"),
            "Should generate ServiceConfig struct"
        );

        // Should contain nested structs
        assert!(
            generated_code.contains("pub struct Service") || generated_code.contains("service:"),
            "Should handle service object"
        );
        ...
        assert!(
            ...
            "Should generate Vec types for arrays"
        );
        assert!(
            generated_code.contains("ServiceConfigServiceEndpointsItem")
                || generated_code.contains("Endpoint")
                || generated_code.contains("endpoints"),
            "Should handle endpoint arrays"
        );

        // Should handle HTTP method enums
        assert!(
            generated_code.contains("GET") || generated_code.contains("Get"),
            "Should handle HTTP methods"
        );

        // Should handle oneOf unions appropriately (may map to serde_json::Value)
        assert!(
            generated_code.contains("serde_json::Value") || generated_code.contains("Properties"),
            "Should handle oneOf unions"
        );

        // Should compile without errors
        assert!(
            validate_rust_syntax(&generated_code),
            "Complex generated code should have valid Rust syntax"
        );
    }
    ...
    fn test_enum_generation_with_various_types() {
        ...
        fs::write(
            &schema_file,
            serde_json::to_string_pretty(&enum_schema).unwrap(),
        )
        .unwrap();
        ...
        assert!(
            generated_code.contains("pub enum") || generated_code.contains("StringEnum"),
            "Should handle string enums"
        );

        // String enum should have proper variants
        assert!(
            generated_code.contains("Active") || generated_code.contains("active"),
            "Should have enum variants"
        );

        // Should handle serde rename for string enums
        assert!(
            generated_code.contains("#[serde(rename") || generated_code.contains("active"),
            "Should handle serde renaming"
        );

        // Mixed field should be handled as a regular field (no enum for mixed types)
        assert!(
            generated_code.contains("mixed_enum"),
            "Should have mixed_enum field"
        );
    }
            ...
            match result {
                Ok(gen_result) => {
                    if gen_result.is_ok() {
                        ...
                        assert!(
                            !code.is_empty(),
                            "If generation succeeds, should produce some code"
                        );
                    } else {
    ...
    fn test_large_schema_performance() {
        ...
        fs::write(
            &schema_file,
            serde_json::to_string_pretty(&large_schema).unwrap(),
        )
        .unwrap();
        ...
        assert!(
            duration.as_secs() < 10,
            "Large schema should generate within 10 seconds, took {:?}",
            duration
        );
        ...
        assert!(
            generated_code.contains("field_0"),
            "Should contain first field"
        );
        assert!(
            generated_code.contains("field_99"),
            "Should contain last field"
        );

        // Should be well-formatted
        assert!(
            generated_code.contains("pub struct LargeSchema"),
            "Should have main struct"
        );
    }
    ...
    fn test_schema_parsing_integration() {
        ...
        fs::write(
            &schema_file,
            serde_json::to_string_pretty(&parser_test_schema).unwrap(),
        )
        .unwrap();
        ...
        assert!(
            schema.properties.contains_key("simple_string"),
            "Should parse simple string property"
        );
        assert!(
            schema.properties.contains_key("constrained_number"),
            "Should parse constrained number"
        );
        assert!(
            schema.properties.contains_key("array_field"),
            "Should parse array field"
        );
        assert!(
            schema.properties.contains_key("object_field"),
            "Should parse object field"
        );
        assert!(
            schema.properties.contains_key("union_field"),
            "Should parse union field"
        );

        // Should parse required fields
        assert!(
            schema.required.contains(&"simple_string".to_string()),
        ...
        assert!(
            result.is_ok(),
            "Code generation from schema file should succeed"
        );

        let generated_code = result.unwrap();
        assert!(
            generated_code.contains("ParserTest"),
            "Should generate struct from parsed schema"
        );
    }
    ...
    fn validate_rust_syntax(code: &str) -> bool {
        // Basic syntax checks
        let brace_balance = code.chars().fold(0i32, |acc, c| match c {
            '{' => acc + 1,
            '}' => acc - 1,
            _ => acc,
        });

        let paren_balance = code.chars().fold(0i32, |acc, c| match c {
            '(' => acc + 1,
            ')' => acc - 1,
            _ => acc,
        });
