...
impl SchemaValidator {
    ...
    fn validate_required_fields(&self, schema: &Schema, required_fields: &[String]) -> Result<(), SchemaError> {
    ...
    fn validate_composition_schemas(&self, schema: &Schema) -> Result<(), SchemaError> {
        ...
        for (i, composition_schema) in schema.all_of.iter().enumerate() {
            ...
            if !composition_schema.properties.is_empty() || composition_schema.schema_type != SchemaType::Any {
                self.validate_schema(composition_schema)
                    .map_err(|e| SchemaError::ValidationError {
        ...
        for schema in any_of_schemas {
            match &schema.schema_type {
                SchemaType::String | SchemaType::Number | SchemaType::Integer | SchemaType::Boolean => {
                    ...
                    if !primitive_types.iter().any(|t| std::mem::discriminant(t) == std::mem::discriminant(&schema.schema_type)) {
...
mod tests {
    ...
    fn test_validate_any_of_composition() {
        ...
        let result = validator.validate_schema(&schema);
        assert!(result.is_ok(), "anyOf schema validation failed: {:?}", result);
    }
    ...
    fn test_validate_one_of_basic_functionality() {
        ...
        schema.properties.insert("data".to_string(), Schema::new(SchemaType::String));
        schema.properties.insert("error".to_string(), Schema::new(SchemaType::Object));
