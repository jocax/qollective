...
mod validation_error_handling_tests {
    ...
    fn test_envelope_validation_provides_detailed_error_messages() {
        ...
        assert!(!schema.is_valid(&invalid_envelope), "Invalid envelope should fail validation");
        ...
        assert!(
            error_messages.iter().any(|msg| msg.contains("timestamp") || msg.contains("date-time")),
            "Should have timestamp format error"
        );
        assert!(
            error_messages.iter().any(|msg| msg.contains("requestId") || msg.contains("uuid")),
            "Should have requestId format error"
        );
        assert!(
            error_messages.iter().any(|msg| msg.contains("version") || msg.contains("pattern")),
    ...
    fn test_envelope_validation_reports_missing_required_fields() {
        ...
        // ACT: Validate and collect errors
        assert!(!schema.is_valid(&incomplete_envelope), "Incomplete envelope should fail validation");
        ...
        assert!(
            error_messages.iter().any(|msg| msg.contains("requestId") || msg.contains("required")),
            "Should report missing requestId field"
        );
        assert!(
            error_messages.iter().any(|msg| msg.contains("version") || msg.contains("required")),
    ...
    fn test_service_config_validation_provides_clear_field_errors() {
        ...
        // ACT: Validate and collect errors
        assert!(!schema.is_valid(&invalid_config), "Invalid service config should fail validation");
        ...
        assert!(
            error_messages.iter().any(|msg| msg.contains("Invalid-Service-Name") && msg.contains("match")),
            "Should report service name pattern error"
        );
        assert!(
            error_messages.iter().any(|msg| msg.contains("invalid-version") && msg.contains("match")),
            "Should report version pattern error"
        );
        assert!(
            error_messages.iter().any(|msg| msg.contains("email") || msg.contains("format")),
            "Should report email format error"
        );
        assert!(
            error_messages.iter().any(|msg| msg.contains("sampling_rate") || msg.contains("maximum")),
    ...
    fn test_security_meta_validation_reports_permission_format_errors() {
        ...
        // ACT: Validate and collect errors
        assert!(!schema.is_valid(&invalid_security), "Invalid security meta should fail validation");
        ...
        assert!(error_count >= 4, "Should have at least 4 permission format errors, got {}", error_count);
    }
    ...
    fn test_validation_error_contains_instance_path() {
        ...
        assert!(!schema.is_valid(&nested_invalid), "Nested invalid data should fail validation");
        ...
        let has_service_chain_errors = errors.iter().any(|error| {
            error.instance_path.to_string().contains("serviceChain")
        });
        
        assert!(has_service_chain_errors, "Should have errors with serviceChain path information");
    }
    ...
    fn test_validation_error_schema_path_information() {
        ...
        assert!(!schema.is_valid(&constraint_violation), "Constraint violation should fail validation");
        
        let errors: Vec<ValidationError> = schema.iter_errors(&constraint_violation).collect();
        assert!(!errors.is_empty(), "Should have constraint violation errors");
        
        // Check that errors include schema path information
        let has_schema_path = errors.iter().any(|error| {
            !error.schema_path.to_string().is_empty()
        });
        
        assert!(has_schema_path, "Errors should include schema path information");
    }
    ...
    fn test_multiple_schema_validation_error_aggregation() {
        ...
        let test_data = vec![
            ...
            ("service_config", &service_config_schema, json!({"invalid": "config"})),
            ("security_meta", &security_meta_schema, json!({"invalid": "security"})),
        ];

        // ACT & ASSERT: Validate each and collect error information
        for (schema_name, schema, data) in test_data {
            assert!(!schema.is_valid(&data), "{} schema should reject invalid data", schema_name);
            
            let errors: Vec<ValidationError> = schema.iter_errors(&data).collect();
            assert!(!errors.is_empty(), "{} schema should produce validation errors", schema_name);
    ...
    fn test_validation_error_provides_helpful_suggestions() {
        ...
        for (i, mistake_data) in common_mistakes.iter().enumerate() {
            ...
            assert!(!schema.is_valid(mistake_data), "Common mistake {} should fail validation", i);
            
            let errors: Vec<ValidationError> = schema.iter_errors(mistake_data).collect();
            assert!(!errors.is_empty(), "Common mistake {} should have errors", i);
    ...
    fn test_validation_performance_with_large_errors() {
        ...
        assert!(validation_duration.as_millis() < 1000, "Validation should complete within 1 second");
        
        let errors: Vec<ValidationError> = schema.iter_errors(&many_errors_data).collect();
        assert!(errors.len() > 10, "Should collect multiple validation errors");
    }
    ...
    fn load_security_meta_schema() -> Validator {
        let schema_path = Path::new("../schemas/core/meta-security.json");
        let schema_content = fs::read_to_string(schema_path)
