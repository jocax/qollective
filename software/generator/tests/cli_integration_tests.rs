...
fn test_cli_validate_with_nonexistent_file() {
    let output = Command::new("cargo")
        .args(&["run", "--", "validate", "nonexistent_schema.json", "--quiet"])
        .output()
        .expect("Failed to run CLI command");
...
fn test_cli_validate_with_invalid_json() {
    ...
    let output = Command::new("cargo")
        .args(&["run", "--", "validate", temp_file.to_str().unwrap(), "--quiet"])
        .output()
        .expect("Failed to run CLI command");
...
fn test_cli_init_command() {
    ...
    let output = Command::new("cargo")
        .args(&[
            "run", "--", "init", "test_project", 
            "--directory", project_dir.to_str().unwrap(),
            "--template", "minimal",
            "--quiet"
        ])
        .output()
